...
table.Monthly <- function(R, digits = 1, as.perc = TRUE, geometric = TRUE){
  ...
  month = format(strptime(as.POSIXct(time(ri)), "%Y-%m-%d"), 
                 "%b")
  monthlabels = strftime(seq.Date(as.Date("2000-01-01"), length.out = 12, 
                                  by = "months"), format = "%b")
  ...
  for (column in 1:columns) {
    ...
    for (i in 1:length(yearcol[, 1])) {
      ...
      }else{yearcol[i, columnnames[column]] = sum(as.numeric(target.df[i,]), na.rm = TRUE)}
