...
updateWind <- function(sym, startDate, endDate, indicators,dataPath=dataPath) {
  ...
  if (nMatch==0) {
    ...
    new.ind <- union(c("open","high","low","close"),indicators)
    ...
    assign(sym,y)
    save(list=c(sym),file=paste0(sym,".RData"))
